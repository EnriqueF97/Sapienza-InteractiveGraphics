...
bool IntersectRay( inout HitInfo hit, Ray ray )
{
	...
	// this for loop is used to find the closest intersection point between the ray and the spheres
	for ( int i=0; i<NUM_SPHERES; ++i ) {
		Sphere sphere = spheres[i];

		// TO-DO: Test for ray-sphere intersection
		// What this does is to find the closest intersection point between the ray and the sphere
		// and update the hit information with the intersection point, normal and material
		float discriminant = pow(dot(ray.dir, (ray.pos - sphere.center)), 2.0) - 
			(dot(ray.dir, ray.dir) * (dot((ray.pos - sphere.center), (ray.pos - sphere.center)) - pow(sphere.radius, 2.0))); 
...
vec3 Shade( Material mtl, vec3 position, vec3 normal, vec3 view )
{
	...
	for ( int i=0; i<NUM_LIGHTS; ++i ) {
		...
		// TO-DO: Check for shadows using every light
		...
		// surfaceToLightRay.pos = position + (surfaceToLightRay.dir) * 0.0001; // Test shadows
		if( IntersectShadowRay(surfaceToLightRay) ){
			// color += vec3(1.0,0.0,0.0);	// Test Shadows
			color += ambientComponent;
		}else{
			// color += vec3(0.0,1.0,0.0);	// Test Shadows
			// TO-DO: If not shadowed, perform shading using the Blinn model
			...
			// color += vec3(0.0,0.0,1.0);	// Test Shadows
